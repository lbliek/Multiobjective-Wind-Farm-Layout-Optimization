...
# 给定中心点 + 半径范围，随机画一个不规则多边形：这个函数本身不是专门生成 feasible 或 reservoir，
# 只是一个底层工具函数，谁调用它，它就给谁生成一个多边形
...
def random_feasible_polygon_inside_unit(config: GeneratorConfig, rng: np.random.Generator) -> Polygon:
    ...
    if not UNIT_SQUARE.contains(poly): #原始生成出来的 feasible 多边形，必须完全在 1×1 里面，否则直接丢弃
        return Polygon()
